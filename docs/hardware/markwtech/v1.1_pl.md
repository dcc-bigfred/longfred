...
| TinyC6 | Co podłączyć |
|--------|----------------|
| Gniazdo **JST** od **spodu** płytki | LiPo **+** / **−** dopiero po **zmierzeniu** polaryzacji (nie ufaj kolorom). Jeśli pod spodem nie ma miejsca na wtyczkę — nie używaj JST, idź na goldpiny poniżej. |
| USB-C | Ładowanie i wgrywanie firmware. Na tej płytce USB i bateria mogą być podłączone naraz. |
| `3V3` (J3-4) | OLED `VCC` i enkoder `+` |
| `GND` (J3-2 i/lub J4-10) | Masa każdego modułu i druga nóżka każdego przycisku. W wariancie goldpin: też **minus ogniwa** na J3-2. |
| `BAT` (J3-1) | Alternatywa dla JST **+**: plus ogniwa, gdy JST od spodu nie wchodzi. Albo druga strona kołyskowego na plusie. Jeśli ogniwo siedzi w JST i nie ma kołyskowego — zostaw wolny. |

Jeśli ogniwo ma trzeci przewód **NTC / biały**, zostaw go niepodłączony i zaizoluj.

Opcjonalny wyłącznik główny: zawsze szeregowo na **plusie**. Przy JST: między plusem ogniwa a JST `+`. Przy goldpinach: między plusem ogniwa a J3-1 `BAT`. Minus idzie prosto do JST `−` albo do J3-2 `GND` — nie przełączaj samej masy.

Wybierz **jedną** drogę ogniwa: para w JST, **albo** para na J3-1/`BAT` + J3-2/`GND`. Nie obie naraz.

**Nie** dokładasz TP4056, Pololu ani dzielnika 47 kΩ. TinyC6 już ładuje ogniwo, reguluje 3,3 V i mierzy baterię na GPIO 4 (442 kΩ / 160 kΩ). Firmware czyta ADC z kalibracją krzywej (miliwolty na nóżce) i mnoży je przez stosunek rezystorów, czyli współczynnik **3,76**.
...
### Klawiatura membranowa 3×4 (7 żył; taśma 8-pin)

Wiersze są **wyjściami**. Kolumny są **wejściami** z wewnętrznym podciąganiem.

Firmware zostaje na GPIO 21/20/19/18 (wiersze) i 7/6/5 (kolumny). **Nie wpinaj taśmy 1:1 w J4-1…J4-7** — kolejność żył nie jest R0…R3, C0…C2.

Numery pinów taśmy z pomiaru (zwarcie przy wciśniętym klawiszu). Pin **1** nie bierze udziału w żadnym klawiszu — zostaw wolny.

| Pin taśmy | Rola | Klawisze | TinyC6 |
|-----------|------|----------|--------|
| 1 | — | nieużywany | nie podłączaj |
...
```text
  pin taśmy:  1    2    3    4    5    6    7    8
  rola:      (—)  C1   R0   C0   R3   C2   R2   R1
  GPIO:           6    21    7   18    5   19   20
  otwór:        J4-6 J4-1 J4-5 J4-4 J4-7 J4-3 J4-2
```

Pomiary: `3+4=1`, `3+2=2`, `3+6=3`, `8+4=4`, `8+2=5`, `8+6=6`, `7+4=7`, `7+2=8`, `7+6=9`, `5+4=*`, `5+2=0`, `5+6=#`.

Jeśli cyfry nadal się mylą, sprawdź czy pin 1 taśmy liczysz od tej samej krawędzi co przy pomiarze — nie zmieniaj numerów GPIO w firmware.
...
| # | Skąd | Dokąd | Po co |
|---|------|-------|-------|
| 1 | LiPo **+** (zmierzony) | TinyC6 JST **+** *albo* J3-1 `BAT` *albo* pin A kołyskowego | ogniwo do wbudowanej ładowarki |
| 2 | LiPo **−** (zmierzony) | TinyC6 JST **−** *albo* J3-2 `GND` | powrót ogniwa — **musi** być, inaczej płytka nie wstanie na baterii |
| — | LiPo biały/NTC | *zaizoluj, zostaw wolny* | JST TinyC6 nie ma pinu NTC |
| 3 | *(opcjonalnie)* pin A kołyskowego | TinyC6 JST **+** *albo* J3-1 `BAT` | wyłącznik główny szeregowo na + |
| 4 | *(opcjonalnie)* LiPo **+** | pin B kołyskowego | druga strona kołyskowego |

Bez kołyskowego nie ma przewodów 3–4: plus ogniwa idzie prosto do JST `+` albo do J3-1 `BAT`. Minus i plus muszą iść **tą samą drogą**: oba w JST (spód), albo oba na goldpinach J3-1 / J3-2 (krawędź).
...
Numery „pin taśmy” jak w pomiarze (pin 1 wolny).

| # | Skąd | Dokąd | Po co |
|---|------|-------|-------|
| 14 | Taśma **pin 3** (`R0`) | TinyC6 GPIO **21** (J4-1) | wiersz `1` `2` `3` |
| 15 | Taśma **pin 8** (`R1`) | TinyC6 GPIO **20** (J4-2) | wiersz `4` `5` `6` |
| 16 | Taśma **pin 7** (`R2`) | TinyC6 GPIO **19** (J4-3) | wiersz `7` `8` `9` |
| 17 | Taśma **pin 5** (`R3`) | TinyC6 GPIO **18** (J4-4) | wiersz `*` `0` `#` |
| 18 | Taśma **pin 4** (`C0`) | TinyC6 GPIO **7** (J4-5) | kolumna `1` `4` `7` `*` |
| 19 | Taśma **pin 2** (`C1`) | TinyC6 GPIO **6** (J4-6) | kolumna `2` `5` `8` `0` |
| 20 | Taśma **pin 6** (`C2`) | TinyC6 GPIO **5** (J4-7) | kolumna `3` `6` `9` `#` |
...
Gniazdo JST TinyC6 ma **2 piny** i siedzi **od spodu** płytki (strona przeciwna do USB-C i nadruków na listwach). Plus idzie do pinu oznaczonego `+` / plus baterii na nadruku; **drugi pin to minus**. W razie wątpliwości [schemat UM](https://github.com/UnexpectedMaker/esp32c6). Jeśli pod spodem nie ma miejsca na wtyczkę — nie walcz z JST; w Kroku 2 weź goldpiny.
...
Jeśli w Kroku 2 podłączysz ogniwo na goldpinach, **minus ogniwa też wejdzie w J3-2**. To nadal ta sama masa — nie szukaj osobnego „minusa baterii” na innym pinie. Przy ciasnocie: minus ogniwa zostaw na J3-2, a gwiazdę OLED/enkodera/przycisków przenieś na J4-10.

### Krok 2 — bateria (i opcjonalny kołyskowy)

Ogniwo ma **dwa** przewody i **oba** muszą trafić na TinyC6: plus *i* minus. Sam plus nic nie zasila.

Gniazdo JST i pady baterii są **od spodu** płytki. Goldpiny `BAT` / `GND` są **na krawędzi** (prawy rząd, od USB w dół: najpierw `BAT`, zaraz pod nim `GND`). Wybierz **jedną** drogę — nie wpinaj ogniwa jednocześnie w JST i w listwę.

```text
Wariant A — JST od spodu              Wariant B — goldpiny na krawędzi
                                      (USB-C u góry, prawy rząd)

  LiPo + ──────► JST +                  LiPo + ──────► J3-1  BAT
  LiPo − ──────► JST −                  LiPo − ──────► J3-2  GND
```

Minus to **zawsze** sąsiad plusa na tej samej drodze: drugi pin JST, albo otwór `GND` tuż pod `BAT`. Nie zostawiaj minusa w powietrzu.

1. **Jeszcze bez OLED, enkodera, klawiatury i przycisków.**
2. NTC (biały/żółty) zostaw zaizolowany.
3. Zrób **wariant A albo B** poniżej. *(przewody 1–2, opcjonalnie 3–4)*

#### Wariant A — wtyczka JST od spodu

Użyj tego, gdy pod płytką jest miejsce na wtyczkę (albo gdy JST jest już wlutowane i chcesz z niego korzystać).

Minus **musi** iść do pinu minusowego JST. Nie zostawiaj go w powietrzu i nie dawaj na `3V3` ani `5V`.

1. Odwróć płytkę. JST ma dwa piny: nadruk `+` / plus baterii to **plus**; **sąsiedni pin to minus** (masa ogniwa).
2. Zmierzony **+** ogniwa → JST **+**.
3. Zmierzony **−** ogniwa → JST **−**. To jedyne miejsce minusa w tym wariancie.
4. Nie dokładaj drugiego przewodu minusa na J3-2: JST `−`, J3-2 `GND` i J4-10 są już zwarte wewnątrz płytki.

#### Wariant B — goldpiny, gdy od spodu nie ma miejsca

Użyj tego, gdy wtyczka JST nie mieści się pod płytką, JST nie jest wlutowane, albo chcesz wyprowadzić ogniwo na bok. Para ogniwa siedzi wtedy na **dwóch sąsiednich otworach** prawego rzędu.

1. USB-C u góry. Prawy rząd, od USB w dół: otwór 1 ma nadruk **`BAT`**, otwór 2 ma nadruk **`GND`**.
2. Zmierzony **+** ogniwa → **J3-1 `BAT`**.
3. Zmierzony **−** ogniwa → **J3-2 `GND`** (tuż pod `BAT`). Minus idzie **obok plusa na tej samej listwie**, nie „gdziekolwiek jest masa” na J4.
4. **Nie wpinaj jednocześnie w JST.** `BAT` na listwie to ta sama sieć co JST `+`; `GND` to ta sama sieć co JST `−`.

Jeśli w Kroku 1 J3-2 ma już wiązkę masy: minus ogniwa może dzielić ten otwór (to ta sama masa). Przy braku miejsca na wtyczkę: minus ogniwa na J3-2, masy modułów na J4-10.

#### Opcjonalny kołyskowy

Zawsze szeregowo na **plusie**. Minus bez wyłącznika.

- Wariant A: LiPo **+** → kołyskowy → JST **+**; LiPo **−** → JST **−**.
- Wariant B: LiPo **+** → kołyskowy → J3-1 `BAT`; LiPo **−** → J3-2 `GND`.

Nie przełączaj samej masy.

**Dlaczego minus nie może wisieć:** bez powrotu ogniwa ładowarka i stabilizator nie domykają obwodu — manipulator nie wstanie na baterii.
...
Siedem przewodów. Taśma ma osiem pozycji; **pin 1 zostaw wolny**. Nie wpinaj obudowy 1:1 w J4. *(przewody 14–20)*

| Pin taśmy | Dokąd | Klawisze |
|-----------|-------|----------|
...
Złe cyfry po starcie → sprawdź numerację pinu 1 taśmy i przełóż przewody, nie ruszaj firmware.
...
5. **Bateria:** ładuj przez USB-C TinyC6, aż dioda ładowania na płytce skończy. Pełne ogniwo powinno pokazać ~4,2 V / 100 % i ADC koło 2100–2600. Tylko gdy tak nie jest, skopiuj `sug` z Diagnostyki i wgraj:
...
- **Zmierz polaryzację LiPo** zanim wpinasz w JST albo w J3-1/`BAT` i J3-2/`GND`. Odwrotna polaryzacja może zabić wbudowaną ładowarkę.
- **Zaizoluj przewód NTC.**
- **Nie lutuj do woreczka ogniwa.** Albo gotowa wtyczka JST od spodu, albo przewody na goldpiny J3-1/`BAT` i J3-2/`GND`.
...
- Ogniwo LiPo 3,7 V, ~1200 mAh. Wtyczka **2-pin JST-PH** jeśli idziesz na gniazdo od spodu; albo luźne przewody / Dupont na J3-1/`BAT` i J3-2/`GND`, gdy pod płytką nie ma miejsca. Sprawdź polaryzację.
...
J4 lewa (12)                         J3 prawa (11)
 1  GPIO21  klawiatura R0            1  BAT     plus ogniwa (gdy nie JST) / kołyskowy / wolny
 2  GPIO20  klawiatura R1            2  GND     masa wspólna; minus ogniwa przy goldpinach
