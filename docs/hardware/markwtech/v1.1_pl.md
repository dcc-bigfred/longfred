...
5. **Bateria:** ładuj przez USB-C TinyC6, aż dioda ładowania na płytce skończy. Z logu skopiuj `suggested_factor` z linii `battery: raw=… suggested_factor=…` i wgraj z tą wartością — nie edytuj mapy pinów:

   `make flash VARIANT=markwtech-v1-1 BATTERY_FACTOR=<suggested_factor>`

   Wartość startowa (mapa pinów) to **3,76**. Sprawdź 100 % na pełnym ogniwie.
