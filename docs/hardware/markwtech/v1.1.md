...
5. **Battery:** charge through TinyC6 USB-C until the onboard charge LED finishes. From the log, copy `suggested_factor` from `battery: raw=… suggested_factor=…` and flash with that value — do not edit the pin map:

   `make flash VARIANT=markwtech-v1-1 BATTERY_FACTOR=<suggested_factor>`

   Starting value (pin map) is **3.76**. Then check 100 % on a full cell.
