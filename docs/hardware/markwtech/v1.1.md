...
|--------|-----------------|
| Onboard **JST** battery socket | LiPo **+** / **−** after you have **measured** polarity (do not trust wire colour) |
...
| `GND` (J3-2 and/or J4-10) | Every module ground and every button’s second leg |
| `BAT` (J3-1) | Optional: only if you add a panel rocker **in series with battery +**. Otherwise leave unused. |

If the cell has a third **NTC / white** lead, leave it unconnected and insulated.

Optional master switch: cut battery **+** and put a KCD1 rocker in series **before** the JST (or between JST `+` and the cell `+`). Do not switch ground only.

**Do not** add TP4056, a Pololu, or a 47 kΩ divider. TinyC6 already charges the cell, regulates 3.3 V, and measures battery on GPIO 4 (442 kΩ / 160 kΩ ≈ factor **3.76** in firmware).
...
| TinyC6 | Firmware | Keypad pin | Matrix role |
|--------|----------|------------|-------------|
| **J4-1** GPIO **21** | `KEYPAD_ROW_PINS[0]` | **R0** | keys `1` `2` `3` |
| **J4-2** GPIO **20** | `KEYPAD_ROW_PINS[1]` | **R1** | keys `4` `5` `6` |
| **J4-3** GPIO **19** | `KEYPAD_ROW_PINS[2]` | **R2** | keys `7` `8` `9` |
| **J4-4** GPIO **18** | `KEYPAD_ROW_PINS[3]` | **R3** | keys `*` `0` `#` |
| **J4-5** GPIO **7** | `KEYPAD_COL_PINS[0]` | **C0** | keys `1` `4` `7` `*` |
| **J4-6** GPIO **6** | `KEYPAD_COL_PINS[1]` | **C1** | keys `2` `5` `8` `0` |
| **J4-7** GPIO **5** | `KEYPAD_COL_PINS[2]` | **C2** | keys `3` `6` `9` `#` |

**Keypad ribbon order is not standardized.** Identify each row/column with a multimeter (a pressed key shorts its row to its column). If digits come out scrambled, swap wires — do not change firmware GPIO numbers.

Those seven holes are consecutive on J4 (USB end). A 7-pin Dupont housing can sit on J4-1…J4-7 in order `21, 20, 19, 18, 7, 6, 5`.
...
| # | From | To | Purpose |
|---|------|-----|---------|
| 1 | LiPo **+** (measured) | TinyC6 JST **+** *or* rocker pin A | cell into onboard charger |
| 2 | LiPo **−** (measured) | TinyC6 JST **−** | cell return |
| — | LiPo white/NTC | *insulate, leave open* | TinyC6 JST has no NTC pin |
| 3 | *(optional)* rocker pin A | TinyC6 JST **+** | master switch in series on + |
| 4 | *(optional)* LiPo **+** | rocker pin B | other side of the rocker |

If there is no rocker, wires 3–4 do not exist: cell + goes straight to JST +.
...
| # | From | To | Purpose |
|---|------|-----|---------|
| 14 | Keypad `R0` | TinyC6 GPIO **21** (J4-1) | row (output) |
| 15 | Keypad `R1` | TinyC6 GPIO **20** (J4-2) | row (output) |
| 16 | Keypad `R2` | TinyC6 GPIO **19** (J4-3) | row (output) |
| 17 | Keypad `R3` | TinyC6 GPIO **18** (J4-4) | row (output) |
| 18 | Keypad `C0` | TinyC6 GPIO **7** (J4-5) | column (input) |
| 19 | Keypad `C1` | TinyC6 GPIO **6** (J4-6) | column (input) |
| 20 | Keypad `C2` | TinyC6 GPIO **5** (J4-7) | column (input) |
...
TinyC6 JST is **2-pin**. Plus goes to the pin marked `+` / battery positive on the board silk (confirm with the [UM schematic](https://github.com/UnexpectedMaker/esp32c6) if the silk is unclear).
...
2. Plug the measured **+** / **−** into the TinyC6 JST (or through the rocker on **+** only). *(wires 1–2, optional 3–4)*
3. Leave NTC insulated.
...
Seven wires. Ribbon order varies — map it with the meter. *(wires 14–20)*

1. Continuity mode. Hold key **`1`**. The pair that beeps is R0 and C0.
2. Hold **`2`**: shared lead is R0, new lead is C1.
3. Continue with `3`, `4`, `7`, `*` until all seven leads are named.
...
Wrong digits after boot → move wires, do not edit firmware.
...
5. **Battery:** charge through TinyC6 USB-C until the onboard charge LED finishes. From the log, copy `suggested_factor` from `battery: raw=… suggested_factor=…` into `BATTERY_CONVERSION_FACTOR` in [`pins_v1_1.rs`](../../../crates/firmware/src/board/variants/markwtech/pins_v1_1.rs). Starting value is **3.76**. Then flash again and check 100 % on a full cell.
...
- **Measure LiPo polarity** before the JST. Reverse polarity can kill the onboard charger.
- **Insulate the NTC lead.**
- **Do not solder to the cell pouch.** Use a proper JST pigtail.
...
- LiPo 3.7 V, ~1200 mAh, **2-pin JST-PH** matching TinyC6 (confirm plug polarity)
...
J4 left (12)                         J3 right (11)
 1  GPIO21  keypad R0                1  BAT     optional switch / unused
 2  GPIO20  keypad R1                2  GND     common ground
