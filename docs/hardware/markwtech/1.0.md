...
> Polish version: [1.0_pl.md](1.0_pl.md) · TinyC6 build: [v1.1.md](v1.1.md)
...
This file is **revision 1.0** (Espressif DevKitC-1). The later TinyC6 board is [v1.1](v1.1.md) — same controls, **different GPIO**, not wiring-compatible.

| Item | Value |
|------|-------|
| Cargo feature | `variant-markwtech` |
| `make` | `VARIANT=markwtech` |
| Descriptor `id` | `markwtech` |
| MCU board | [ESP32-C6-DevKitC-1](https://docs.espressif.com/projects/esp-dev-kits/en/latest/esp32c6/esp32-c6-devkitc-1/user_guide.html) |
| Display | SSD1309/SSD1306 128×64 I2C |
| Expanders | none |
| Console | UART0 `/dev/ttyUSB0` |
...
Constants: `board/variants/markwtech/` (`KEYPAD_MAP`, `EXTRA_BUTTON_MAP`; GPIO in `pins_v1_0.rs`).
...
`BATTERY_CONVERSION_FACTOR` starts at **2.0** for the 47 kΩ/47 kΩ divider ([`pins_v1_0.rs`](../../../crates/firmware/src/board/variants/markwtech/pins_v1_0.rs), re-exported from [`config/power.rs`](../../../crates/firmware/src/config/power.rs)). Charge the cell fully and, only if Diagnostics is not ~4.2 V, copy `sug` from Diagnostics (or `suggested_factor` from `battery: pin_mv=…`) and flash that value:

`make flash VARIANT=markwtech BATTERY_FACTOR=<suggested_factor>`

Expect ADC around `2600` and a factor near `1.6`.

With a working cell the firmware also auto-sleeps: deep sleep below **5 %** charge, and after **15 minutes** of no input when no locomotive is moving (speed 0). Before deep sleep the throttle sends EStop. A **5 minute** input idle blanks the OLED (`DisplayOff`) when no locomotive is moving; any key or encoder detent wakes the panel without performing the action, except Stop/EStop which still emergency-stops. A locomotive with speed > 0 keeps the panel on. Deep-sleep wake remains the encoder `SW` on GPIO 0.
...
4. **Battery calibration:** charge the cell fully through the USB-C port on the charger module (the LED on it will change colour). Diagnostics should show pack voltage near 4.2 V and pin around 2100 mV. Only if it does not, copy `sug` from Diagnostics and flash `make flash VARIANT=markwtech BATTERY_FACTOR=<suggested_factor>`.
...
- [MarkWTech v1.1 (TinyC6)](v1.1.md) — same throttle, Unexpected Maker TinyC6 pin map and onboard charger.
