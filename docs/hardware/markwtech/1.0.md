...
`BATTERY_CONVERSION_FACTOR` starts at **1.7** for the 47 kΩ/47 kΩ divider ([`pins_v1_0.rs`](../../../crates/firmware/src/board/variants/markwtech/pins_v1_0.rs), re-exported from [`config/power.rs`](../../../crates/firmware/src/config/power.rs)). Charge the cell fully and, only if Diagnostics is not ~4.2 V, copy `sug` from Diagnostics (or `suggested_factor` from `battery: raw=…`) and flash that value:
