...
Stała `BATTERY_CONVERSION_FACTOR` startuje od **2,0** dla dzielnika 47 kΩ/47 kΩ ([`pins_v1_0.rs`](../../../crates/firmware/src/board/variants/markwtech/pins_v1_0.rs), re-eksport z [`config/power.rs`](../../../crates/firmware/src/config/power.rs)). Naładuj ogniwo do pełna i tylko gdy Diagnostyka nie pokazuje ~4,2 V, skopiuj `sug` z Diagnostyki (albo `suggested_factor` z `battery: pin_mv=…`) i wgraj:

`make flash VARIANT=markwtech BATTERY_FACTOR=<suggested_factor>`

Spodziewaj się odczytu ADC około `2600` i współczynnika bliskiego `1,6`.

Przy działającym ogniwie firmware sam usypia manipulator: głębokie uśpienie poniżej **5 %** naładowania oraz po **15 minutach** bez inputu, gdy żadna lokomotywa nie jedzie (speed 0). Przed snem idzie EStop. Po **5 minutach** bezczynności gaśnie OLED (`DisplayOff`), gdy żadna lokomotywa nie jedzie; dowolny klawisz lub detent enkodera budzi panel bez wykonania akcji, wyjątek to Stop/EStop (hamulec awaryjny). Lokomotywa z prędkością > 0 trzyma panel włączony. Z głębokiego snu budzi nadal `SW` enkodera na GPIO 0.
...
4. **Kalibracja baterii:** naładuj ogniwo do pełna przez port USB-C ładowarki (dioda na module zmieni kolor). Diagnostyka powinna pokazać napięcie ogniwa koło 4,2 V i pin około 2100 mV. Tylko gdy tak nie jest, skopiuj `sug` z Diagnostyki i wgraj `make flash VARIANT=markwtech BATTERY_FACTOR=<suggested_factor>`.
